...
from api.rag.agents import ToolCall, MCPToolCall, RAGUsedContext, coordinator_agent_node, product_qa_agent_node, shopping_cart_agent_node, Delegation
...
def coordinator_router(state: State) -> str:
    ...
    if state.coordinator_final_answer:
        ...
        return "end"
    if state.next_agent == "product_qa_agent":
        return "product_qa_agent"
    if state.next_agent == "shopping_cart_agent":
